...
#' }
#' @section Weight:
#' The function can produce weighted statistics using the \code{weight} parameter.
#' A weight assigns a relative importance to an observation.
#' To produce weighted statistics, pass the name of the variable that holds the
#' weight to the
#' \code{weight} parameter, quoted or unquoted. This variable will then be used
#' when calculating many of the statistics generated by \code{proc_means}.
#' For example, the "mean", "median", "std", and "vari" statistics will accept
#' weighted values.  On the other hand, "min" and "max" are not effected by
#' the weight. The "skewness" and "kurtosis" statistics also do not
#' not accept weighted values, and will instead produce an NA.
#'
#' @section Options:
...
#' an NA value for the weight will be excluded from the analysis.  Also observe
#' the "vardef" option for weighted variances, as this option can effect
#' which denominator is used for the variance calculation.
